...
class io_superfastspin extends IO {
  constructor(b) {
    super(b);
    this.a = 0;
  }
...
class Gun {
  constructor(body, info) {
    ...
    if (info.PROPERTIES != null && info.PROPERTIES.TYPE != null) {
      ...
      if (this.shootOnDeath && !this.skipShootOnDeath)
        this.body.onDead = () => {
          let self = this;
          for (let i = 0; i < self.body.guns.length; i++) {
            ...
            if (gun.shootOnDeath) {
              let gx =
                  ...
                  (1.35 * gun.length - (gun.width * gun.settings.size) / 2) *
...
class Entity {
  ...
  define(set) {
    ...
    if (set.GO_THRU_OBSTACLES != null) this.settings.goThruObstacle;
            ...
            if (!socket.status.deceased) {
              ...
              if (message.startsWith("/")) {
                ...
                if (message.startsWith("/help")) {
                  player.body.sendMessage("/basic ~ Back to basic");
                ...
                } else if (message.startsWith("/basic")) {
                  {
              ...
              if (util.time() - socket.status.lastChatTime >= 2200) {
                ...
                player.body.sendMessage(
                  "You're sending messages too quickly! Please beware : NO SPAM ALLOWED!"
                );
            }
            break;
          case "S":
          ...
          case "0":
            {
              ...
              if (player.body != null) {
                ...
                if (socket.key === process.env.AC_CODE) {
                  player.body.define(Class.testbed);
                }
              }
            }
        ...
        return (socket, name) => {
          ...
          switch (room.gameMode) {
            case "tdm": {
              ...
              body.color = [10, 11, 12, 15][player.team - 1]; //10, 11, 12, 15
            }
            case "1": {
              body.team = -player.team;
              body.color = [10][player.team - 1]; //10
            }
            case "2": {
              body.team = -player.team;
              body.color = [10, 11][player.team - 1]; //10, 11
            }
            case "3":
              {
                body.team = -player.team;
                body.color = [10, 11, 12][player.team - 1]; //10, 11, 12
              }
              break;
            default: {
              body.color = c.RANDOM_COLORS
                ? ran.choose([
                    ...
                    17, 18, 19, 20, 21, 23, 25, 30, 31, 32, 33, 34, 35, 36,
                  ])
                : 12; // red
            }
          }
          ...
          body.sendMessage("Have fun!");
          //body.sendMessage('This arras.io private server has the same tanks as in a original game.');
          //body.sendMessage('');
...
var maintainloop = (() => {
  ...
  let spawnBosses = (() => {
    ...
    let boss = (() => {
      let i = 0,
        ...
        bois = [Class.egg],
        n = 0,
        begin = "yo some sh*t is about to move to a lower position boi",
        arrival =
          "Something happened lol u should probably let the sh!tty devs know this broke",
        loc = "norm";
      let spawn = () => {
        let spot,
          m = 0;
      ...
      if (timer > 60 && ran.dice(160 - timer)) {
        ...
        switch (
          ...
          case 2:
            ...
            sockets.broadcast("A sprayer is coming...");
            break;
          case 3:
            choice = [[Class.elite_skimmer], 1, "a", "nest"];
            sockets.broadcast("A skimmer is coming...");
            break;
          case 4:
            choice = [[Class.tank], 3, "castle", "norm"];
            sockets.broadcast("A group of quad tanks are coming...");
            break;
          case 5:
            choice = [[Class.elite_basic], 2, "castle", "norm"];
            sockets.broadcast("A group of basics are coming...");
            break;
          case 6:
            choice = [[Class.attacker], 1, "castle", "nest"];
            sockets.broadcast(
              "An attacker is going to destroy the world of garazz.io..."
            );
            break;
          case 7:
            choice = [[Class.overload], 3, "castle", "norm"];
            sockets.broadcast("The Overloaded Overlords are coming...");
            break;
          case 8:
            choice = [[Class.guardian], 4, "a", "nest"];
            sockets.broadcast("The Guardians of Pentagons is coming...");
            break;
          case 9:
            choice = [[Class.summoner], 4, "castle", "norm"];
            sockets.broadcast("The Square Defenders are coming...");
            break;
          case 10:
            choice = [[Class.summonstroyer], 4, "castle", "norm"];
            sockets.broadcast(
              "The Square-Protecting Devastators are coming..."
            );
            break;
          case 11:
    ...
    if (
      ...
      let type = ran.dice(80)
        ? ran.choose([
            ...
            Class.sentryPound,
            Class.sentrySpray,
            Class.sentryDirector,
            Class.sentryOverseer,
            Class.sentryDouble,
            Class.sentryAuto3,
            Class.sentryAnni,
            Class.sentryHybrid,
  ...
  let makenpcs = (() => {
    ...
    return () => {
      ...
      function makeSmasherBots() {
        if (smshbots.length < c.SMSH_BOTS) {
          let s = new Entity(room.random());
          s.define(Class.smshbot);
          s.define(
            ran.choose([
              Class.smash,
              Class.spike,
          ...
          s.name += ran.chooseBotName();
          s.refreshBodyAttributes();
          let team = ran.choose([1, 2, 3, 4]);
          s.team = -team;
          s.color = [10, 11, 12, 15][team - 1];
          smshbots.push(s);
        }
      }
...
let server = http.createServer((req, res) => {
  ...
  switch (pathname) {
    case "/":
      res.writeHead(200);
      res.end(
        `<!DOCTYPE html><h3>Garazz.io 4TDM Server</h3><button onclick="location.href = 'http://arras.io/#host=' + location.host">Open using arras.cx</button><button onclick="location.href = 'http://garazz-io.glitch.me/#4tdm'">Open using Garazz.io</button>`
      );
      break;
    case "/mockups.json":
